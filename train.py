...
def get_arguments():
    ...
    parser.add_argument("--restore-from", type=str, default=RESTORE_FROM,
                        help='Path to a pre-trained model to restore from')
...
if __name__ == '__main__':
    ...
    if local_rank == 0:
        print(datetime.datetime.now(),
              f'INFO: MAE: Start training, Total images: {len(train_loader) * args.batch_size}, Total iters: {args.total_iter}, Total epochs: {num_epoch}, Iters of each epoch: {num_iters}')
    ...
    for i_epoch in range(num_epoch):
        ...
        for i_iter, batch in enumerate(train_loader, start=1):
            ...
            if args.lr_scheduler == "cosine":
                lr = adjust_learning_rate_cos(optimizer, current_iter, args.total_iter, 0, args.learning_rate,
                                              power=1.0, T_max=args.total_iter)
            elif args.lr_scheduler == "step":
                steps = [0, args.total_iter // 2, 3 * args.total_iter // 4, args.total_iter]
                lr = adjust_learning_rate_step(optimizer, current_iter, args.total_iter, 0, args.learning_rate,
                                               steps=steps)
            else:
            ...
            eta_seconds = ((time.time() - start_time) / (current_iter - args.start_iter)) * (
                        args.total_iter - current_iter)
            ...
            if local_rank == 0 and current_iter % 10 == 0:
                print(datetime.datetime.now(),
                      "INFO:HisDoc2022: Iters: {:d}/{:d} || Lr: {:.8f} || Loss: {:.4f} || Time: {} <-- {}".format(
                          current_iter, args.total_iter, lr, loss.item(), time_cost, time_need))
